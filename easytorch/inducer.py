...
from easytorch.module import Model, Criterion, Evaluator, Optimizer

#
# class to manage model training/evaluation
# usage : execute training by train()
# usage : execute evaluation by eval()
class Inducer():

  # initializer
  def __init__(self, **kwargs:dict):
    # initialize member function
    self._model = Model(kwargs.get("model", None))
    self._criterion = Criterion(kwargs.get("criterion", None))
    self._evaluator = Evaluator(kwargs.get("evaluator", None))
    self._optimizer = Optimizer(kwargs.get("optimizer", None))
  ...
  def check_available(self):
    ...
    if (not self._data_loader):
      print("error : data loader is not loaded.")
      sys.exit()
  ...
  def train(self):
    ...
    total_size = 0
    self._model.train()
    for batch in self._data_loader.get_train_data_loader():
  ...
  def eval(self):
    ...
    self._model.eval()
    with torch.no_grad():
      for batch in self._data_loader.get_eval_data_loader():
        out = self._model(**batch)
        loss = self._criterion(**out)
  ...
  def test(self):
    ...
    self._model.eval()
    with torch.no_grad():
      for batch in self._data_loader.get_test_data_loader():
        out = self._model(**batch)
        loss = self._criterion(**out)
        eval = self._evaluator(**out) if self._evaluator.check_available() else 0.0
        total_loss += loss
