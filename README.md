...
You can setup __DataLoader__ class by introducing user-defined train/eval/test dataset class.
And you can also set properties (batch_size & num_workers) related to __torch.utils.data.Dataloader__ class in PyTorch.

```python
data_loader = DataLoader.DataLoader(train_data=ud_train_data, test_data=ud_test_data,
...
Here, it is needed to inhelit __torch.nn.Module__ class and define __forward()__ method in user-defined module class.
...
After that, all components needed for deep learning are taken into __Inducer__ class.

```python
inducer = Inducer.Inducer(model=model, criterion=criterion, optimizer=optimizer, evaluator=evaluator, 
                          data_loader=data_loader, device=device, device_ids=device_ids, non_blocking=True)
```
